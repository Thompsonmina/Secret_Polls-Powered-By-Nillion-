...
def nada_main():
    ...
    starting_val = SecretInteger(Input(name="starting_val", party=participants[0]))

    responses = []
    for i in range(num_particpants):
        responses.append( SecretInteger(Input(name=f"poll_4_p{i + 1}_response", party=participants[i])) )
    ...
    output_poll_results = [
        Output(response, f"option-{option}", party=participants[0])
